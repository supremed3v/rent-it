...
export default function UserDetails() {
  ...
  const getRentedItems = useCallback(async () => {
    try {
      const { data } = await axios.get(
        `http://localhost:5000/api/v1/get-rented-products/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const rows = sellerProducts?.map((product) => {
    return {
      ...
      images: product.images && product.images.map((image) => image[0]?.url),
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            {userDetails?.role === "seller" && (
              <img
                src={
                  userDetails?.verifySellerIdCard?.url === "url"
                    ? "https://picsum.photos/200/300"
                    : userDetails?.verifySellerIdCard?.url
                }
                alt={userDetails?.name}
                style={{
                  width: "300px",
          ...
          <Grid item xs={12} md={6}>
            {userDetails?.role === "seller" && (
              <img
                src={
                  userDetails?.verifySellerImage?.url === "url"
                    ? "https://picsum.photos/200/300"
                    : userDetails?.verifySellerImage?.url
                }
                alt={userDetails?.name}
                style={{
                  width: "300px",
